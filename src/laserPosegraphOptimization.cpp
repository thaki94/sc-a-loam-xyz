...
pcl::PointCloud<PointType>::Ptr laserCloudMapCropped(new pcl::PointCloud<PointType>());
...
ros::Publisher pubMapAftPGO, pubOdomAftPGO, pubPathAftPGO, pubMapCropped;
...
int findNearestIndexBinarySearch(const std::vector<double>& vec, double target)
{
    // Use binary search to find the lower bound (nearest value <= target)
    auto it = std::lower_bound(vec.begin(), vec.end(), target);

    // Check if the lower bound is the first element
    if (it == vec.begin())
        return 0;

    // Check if the lower bound is the last element
    if (it == vec.end())
        return static_cast<int>(vec.size()) - 1;

    // Calculate the differences between lower bound and the element before it
    double diff1 = std::abs(*it - target);
    double diff2 = std::abs(*(it - 1) - target);

    // Return the index of the nearest element
    return (diff1 < diff2) ? static_cast<int>(std::distance(vec.begin(), it))
                           : static_cast<int>(std::distance(vec.begin(), it - 1));
}

void radarHandler(const sensor_msgs::Image::ConstPtr& msg)
{

    ros::Time timestamp = msg->header.stamp;
    double timestamp_sec = timestamp.toSec();

    radar_timestamps.push_back(timestamp_sec);
...
void pubCropped(){
    ...
    int nearestIndex = findNearestIndexBinarySearch(keyframeTimes, radar_timestamps.at(radar_count));
    

    //ROS_INFO("Nearest Lidar idx: %d %lf %lf", nearestIndex, timestamp_sec,keyframeTimes[nearestIndex]);
    cout<<nearestIndex<<" "<<recentIdxUpdated<<" "<<radar_timestamps.size()<<" "<<keyframePosesUpdated.size()<<endl;
    
    if((recentIdxUpdated>nearestIndex+offset) && (nearestIndex-offset>0)){

        cout<<nearestIndex<<" "<<recentIdxUpdated<<" "<<endl;
        laserCloudMapCropped->clear();

        //mKF.lock(); 

        for (int node_idx=nearestIndex-offset; node_idx < nearestIndex+offset; node_idx++) {
            //if(counter % SKIP_FRAMES == 0) {
            //cout<<"HERE !!!!!!!!!!!!!!!!!!!!! "<< node_idx<<endl;
            *laserCloudMapCropped += *local2global(keyframeLaserClouds[node_idx], keyframePosesUpdated[node_idx]);
            //}
            //counter++;
        }
        //mKF.unlock(); 

        downSizeFilterMapPGO.setInputCloud(laserCloudMapCropped);
        downSizeFilterMapPGO.filter(*laserCloudMapCropped);

        //sensor_msgs::PointCloud2 laserCloudMapCroppedMsg;
        //pcl::toROSMsg(*laserCloudMapCropped, laserCloudMapCroppedMsg);
        //laserCloudMapCroppedMsg.header.frame_id = "/camera_init";
        //pubMapCropped.publish(laserCloudMapCroppedMsg);

        pcl::io::savePCDFileASCII("/home/isl/Thakshila/boreas-2021-09-02-11-42/lidar-cropped/"+to_string(radar_count)+".pcd", *laserCloudMapCropped);
    
    }
    //image_timestamps.push_back(timestamp_sec);
    
    //ROS_INFO("Image timestamp: %f", timestamp_sec);
    

    radar_count=radar_count+100;
...
void pubCropped2()
{
    ...
    *Croppedcloud=*laserCloudMapPGO;
    ...
    //if((recentIdxUpdated>nearestIndex+offset) && (nearestIndex-offset>0)){
        ...
        double x = keyframePosesUpdated[nearestIndex].x;
        double y = keyframePosesUpdated[nearestIndex].y;
        double z = keyframePosesUpdated[nearestIndex].z;

        double roll = keyframePosesUpdated[nearestIndex].x;
        double pitch = keyframePosesUpdated[nearestIndex].y;
        double yaw = keyframePosesUpdated[nearestIndex].z;
        ...
        // Crop the point cloud using a CropBox filter
        ...
        Eigen::Affine3f crop_box_transform = Eigen::Affine3f::Identity();
        crop_box_transform.translation() << x, y, z;
        crop_box_transform.rotate(Eigen::AngleAxisf(yaw, Eigen::Vector3f::UnitZ()));
        crop_box_transform.rotate(Eigen::AngleAxisf(pitch, Eigen::Vector3f::UnitY()));
        crop_box_transform.rotate(Eigen::AngleAxisf(roll, Eigen::Vector3f::UnitX()));

        crop.setTransform(crop_box_transform);

        crop.setInputCloud(Croppedcloud);

        crop.filter(*Croppedcloud);


        // Save the cropped point cloud to a PCD file
        pcl::io::savePCDFileASCII("/home/isl/Thakshila/boreas-2021-09-02-11-42/lidar-cropped-2/"+to_string(radar_count)+".pcd", *Croppedcloud);


        sensor_msgs::PointCloud2 laserCloudMapCroppedMsg;
        pcl::toROSMsg(*Croppedcloud, laserCloudMapCroppedMsg);
        laserCloudMapCroppedMsg.header.frame_id = "/camera_init";
        pubMapCropped.publish(laserCloudMapCroppedMsg);
    
    //}
        radar_count=radar_count+20;
}

void gpsPublisher(double* xyz){
    ...
    printf("GPS PUB xyz[0]: %f xyz[1]: %f xyz[2]: %f\n", xyz[0], xyz[1], xyz[2]);
...
void transformXYZ(double* xyz, double* transxyz, double heading)
{

    geometry_msgs::PointStamped original_point;
    original_point.header.frame_id = "original_frame";
    original_point.point.x = xyz[0];
    original_point.point.y = xyz[1];
    original_point.point.z = xyz[2];
   ...
   tf2::Matrix3x3 rotation_matrix;
     ...
     rotation_matrix.setValue(
         7.360555651493132512e-01, -6.769211217083995757e-01, 0,
         6.769211217083995757e-01, 7.360555651493132512e-01, 0,
         0, 0, 1
     );
    ...
    tf2::Vector3 translation(0, 0, 1.300000000000000044e-01);

    //tf2::Vector3 translation(0, 0, 0);
    
    tf2::Transform transformation(rotation_matrix, translation);

    geometry_msgs::TransformStamped transform_stamped;
    transform_stamped.transform = tf2::toMsg(transformation);
    transform_stamped.header.stamp = ros::Time::now();
    transform_stamped.header.frame_id = "target_frame";
    transform_stamped.child_frame_id = "original_frame";
    ...
    transxyz[0] = transformed_point.point.x;
    transxyz[1] = transformed_point.point.y;
    transxyz[2] = transformed_point.point.z;

    gpsPublisher(transxyz);

    //printf("transxyz[0]: %f transxyz[1]: %f transxyz[2]: %f\n", transxyz[0], transxyz[1], transxyz[2]);
}


void GPS2XYZ(double latitude, double longitude, double altitude, double* xyz, char F)
{
    if(F=='Y')
    {
        //printf("HERE\n");    
...
void initNoises( void )
{
    ...
    double bigNoiseTolerentToXY = 0.1; // 1e9
...
void pubPath( void )
{
    ...
    // if(recentIdxUpdated>50 && recentIdxUpdated%10==0 && lastIdx!=recentIdxUpdated){
    //     cout<<"recentIdxUpdated                :"<<recentIdxUpdated<<endl;
    //     pubCropped2();
        
    //     lastIdx=recentIdxUpdated;
...
//    if((recentIdxUpdated>10) && ((recentIdxUpdated%5)==0)){
//        cout<<"Here!!!!!!!!!!!!!!!!"<<endl;
//        pubCropped();
		...
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            if( !gpsOffsetInitialized ) {
                ...
                if(hasGPSforThisKF) { // if the very first frame 
                    ...
                    //GPS2XYZ(currGPS->latitude , currGPS->longitude , currGPS->altitude, xyz,'Y');
                } 
            }
            ...
            if( ! gtSAMgraphMade /* prior node */) {
                ...
                mtxPosegraph.lock();
                {
                    ...
                    if(hasGPSforThisKF) {
                        ...
                        ///////////
                        //GPS2XYZ(currGPS->latitude , currGPS->longitude , currGPS->altitude, xyz,'N');
                        //transformXYZ(xyz,transxyz);

                        xyz[0]=currGPS->latitude;
                        xyz[1]=currGPS->longitude;
                        xyz[2]=currGPS->altitude;

                        heading=currGPS->position_covariance[0];

                        transformXYZ(xyz,transxyz,heading);
                        //////////

                        //cout<<"Transformed:"<<transxyz[0]<<" "<<transxyz[1]<<" "<<transxyz[2]<<endl;

                        //gtsam::Point3 gpsConstraint(recentOptimizedX, recentOptimizedY, transxyz[2]); // in this example, only adjusting altitude (for x and y, very big noises are set) 
                        gtsam::Point3 gpsConstraint(transxyz[0], transxyz[1], transxyz[2]);
...
void pubMap(void)
{
    ...
    pcl::io::savePCDFileASCII("/home/isl/Thakshila/maps/map-with-xyz.pcd", *laserCloudMapPGO);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber subRadar = nh.subscribe("/radar/polar", 1, radarHandler);
    ...
    pubMapCropped = nh.advertise<sensor_msgs::PointCloud2>("/cropped_map", 100);
